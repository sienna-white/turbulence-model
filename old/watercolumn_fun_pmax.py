...
def run_watercolumn(pmax, ws, output_csv):

    t1 = time.time()  # Time our simluation 
    print("Running simulation w/ ws = %e and pmax = %e" % (ws, pmax))

    def save_at_end(result):
        lib.save_output(output_csv, pmax, ws, result, header=["pmax", "ws", "output"])
    ...
    diatoms = Algae_Species(k = 0.07,    # specific light attenuation coefficient [cm^2 / 10^6 cells]
                    pmax = pmax,# 5, #0.1, #0.05,     # maximum specific growth rate [1/hour]
