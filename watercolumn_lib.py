...
class WCModel():
    ...
    ***************************************************************************
    ...
    def wind(self, t):
        return self.forcings["Wind_speed"][int(t)]*2
    ...
    ***************************************************************************
    ...
    def plot_phasing(self, ListOfSpecies, passed_string='', skip=1, show=True):
        ...
        # 
        # diurnal = self.forcings["I_in"] #[diurnal_light(t, I_in, True) for t in time]
        
        print(self.forcings["I_in"][0:self.M])
        print(self.forcings["T_air"][0:self.M])
        
        axs[2].plot(self.forcings["I_in"][0:self.M], label='Light', linewidth = 3, color='yellow')
        ax2 = axs[2].twinx()
        ax2.plot(self.forcings["T_air"][0:self.M], label='Temperature', linewidth = 3, color='crimson')
        ax2.plot(self.forcings["Wind_speed"][0:self.M], label='Wind', linewidth = 3, color='skyblue')
