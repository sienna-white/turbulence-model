...
bottom_temp = 29.5
del_temp = 1 # float(os.environ["del_temp"])
top_temp =  del_temp + bottom_temp
bottom_speed = 0 
top_speed=3.5  
...
########################################################################################## 
output_csv = os.environ["output_csv"] # "PHASING_OUTPUT.csv" # Sw edit
...
def wc_advance(Up, Cp, Q2p, Q2Lp, rhop, Lp, nu_tp, Kzp, Kqp, N_BV2p, Ap1, Ap2, time_index):
    ...
    Light =  model.diurnal_light(time_index, I_max, phase_shift=LIGHT_PHASE_SHIFT, diurnal=True)
...
attributes = {"Px0": Px0, "T_Px": T_Px, "I_in": I_max, "Diurnal" : int(DIURNAL_LIGHT),
...
# edit sw
# save_at_end(del_temp, WIND_PHASE_SHIFT, sum(algae1), sum(algae2))
save_at_end(TIDAL_PHASE_SHIFT, WIND_PHASE_SHIFT, sum(algae1), sum(algae2))


print("Total time = %f" % (time.time()  - t1))

f0, a0 = model.plot_shear(skip=1,passed_string=TITLE, show=True)
f0.savefig('figures//phase/poster/%s_Shear.png' % TITLE)

f0, a0 = model.plot_profiles('C', skip=1, passed_string='', show=True)
f0.savefig('figures/phase/poster/%s_Temp.png' % TITLE)

print("Saving...", TITLE)
f0, a0 = model.plot_profiles('U', skip=1, passed_string='', show=True)
f0.savefig('figures/phase/poster/%s_U.png' % TITLE)

f0, a0 = model.plot_profiles('Kz', skip=1, passed_string='', show=True)
f0.savefig('figures/phase/poster/%s_KZ.png' % TITLE)

f0, a0 = model.plot_profiles('C', skip=1, passed_string='', show=True)
f0.savefig('figures/phase/poster/%s_C.png' % TITLE)

f0, a0 = model.plot_profiles('N_BV2', skip=1, passed_string='', show=True)
f0.savefig('figures/phase/poster/%s_N_BV2.png' % TITLE)

f0, a0 = model.plot_profiles('algae1', skip=1, passed_string=TITLE, show=True)
f0.savefig('figures/phase/poster/%s_algae1.png' % TITLE)

f0, a0 = model.plot_profiles('algae2', skip=1, passed_string=TITLE, show=True)
f0.savefig('figures/phase/poster/%s_algae2.png' % TITLE)
...
f0, a0 = model.plot_phasing([Algae1, Algae2], pressure=Px, light=light, wind=wind, passed_string=TITLE, skip=1, show=True)

plt.suptitle('%s' % TITLE)
f0.savefig('figures/phase/poster/%s_PHASING.png' % TITLE)
